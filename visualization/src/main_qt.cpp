    ...
    std::atomic<bool>& isRunning) 
{
...

    {
        ...
        sharedFeat->metrics.samples_processed = 0;
    ...
    while (isRunning && start + params.fft_size <= data.sample_count) {
        ...
        // 1. Parallel Constellation Processing
        auto constel_future = std::async(std::launch::async, [&extractor, chunk_buf]() {
            return extractor.extract_time(chunk_buf);
        });

        // 2. Main Thread Waterfall Processing
        ...
        // 3. Merge Results
        auto current_features = constel_future.get();
        extractor.extract_spectral(row, current_features);
...
public:
    DashboardApp(QWidget* parent = nullptr) : QMainWindow(parent) {
        setWindowTitle(QString::fromUtf8("NTRO Unified Stage 4 \xe2\x80\x93 Signal Analysis Dashboard"));
        resize(1920, 1080);
...
private:
    ...
    void onStartAnalysisClicked() {
        ...
        connect(m_btnPlay, &QPushButton::clicked, m_ui.constellationWidget, &ConstellationPlotWidget::play);
        connect(m_btnPause, &QPushButton::clicked, m_ui.waterfallWidget, &WaterfallPlotWidget::pause);
        connect(m_btnPause, &QPushButton::clicked, m_ui.constellationWidget, &ConstellationPlotWidget::pause);
    ...
    void initDashboardScreen() {
        ...
        QLabel* lblTitle = createLabel(QString::fromUtf8("NTRO Unified Stage 4 \xe2\x80\x93 Signal Analysis Dashboard"), "#FFFFFF", true, 20);
        navLayout->addWidget(lblTitle);
        navLayout->addStretch(1);

        auto createStatus = [](const QString& name) {
            QLabel* l = createLabel(name + QString::fromUtf8(" \xe2\x97\x8f ACTIVE"), "#888888", true, 13);
            return l;
        };
        ...
        m_btnPlay = new QPushButton(QString::fromUtf8("\xe2\x96\xb6 PLAY"));
        m_btnPause = new QPushButton(QString::fromUtf8("\xe2\x8f\xb8 PAUSE"));
        ...
        m_ui.lblPipeline = createLabel(QString::fromUtf8("\xe2\x9c\x93 INPUT \xe2\x86\x92 \xe2\x9c\x93 mmap() \xe2\x86\x92 \xe2\x9c\x93 BUFFER \xe2\x86\x92 \xe2\x9c\x93 DSP \xe2\x86\x92 \xe2\x9c\x93 VISUALIZATION"), "#00FF00", true, 13);
    ...
    void updateDashboard() {
        ...
        bool isWorking = (m.samples_received > 0);
        auto setStatus = [](QLabel* l, bool active, const QString& name) {
            if (active) {
                l->setText(name + QString::fromUtf8(" \xe2\x97\x8f ACTIVE"));
                l->setStyleSheet("color: #00FF00; font-size: 13px; font-weight: bold;");
            } else {
                l->setText(name + QString::fromUtf8(" \xe2\x97\x8f INACTIVE"));
        ...
        bool mmapVerified = m.mmap_active && m.samples_received > 0 && m.mapped_address != 0;
        if (mmapVerified) {
            m_ui.lblPipeline->setText(QString::fromUtf8("\xe2\x9c\x93 INPUT \xe2\x86\x92 \xe2\x9c\x93 mmap() \xe2\x86\x92 \xe2\x9c\x93 BUFFER \xe2\x86\x92 \xe2\x9c\x93 DSP \xe2\x86\x92 \xe2\x9c\x93 VISUALIZATION"));
            m_ui.lblPipeline->setStyleSheet("color: #00FF00; font-size: 13px; font-weight: bold;");
        } else if (m.mmap_active && m.samples_received == 0) {
            m_ui.lblPipeline->setText(QString::fromUtf8("\xe2\x9c\x93 INPUT \xe2\x86\x92 \xe2\x9a\xa0 mmap() \xe2\x86\x92 \xe2\x9c\x95 BUFFER \xe2\x86\x92 \xe2\x9c\x95 DSP \xe2\x86\x92 \xe2\x9c\x95 VISUALIZATION"));
            m_ui.lblPipeline->setStyleSheet("color: #FFFF00; font-size: 13px; font-weight: bold;");
        } else {
            m_ui.lblPipeline->setText(QString::fromUtf8("\xe2\x9c\x95 INPUT \xe2\x86\x92 \xe2\x9c\x95 mmap() \xe2\x86\x92 \xe2\x9c\x95 BUFFER \xe2\x86\x92 \xe2\x9c\x95 DSP \xe2\x86\x92 \xe2\x9c\x95 VISUALIZATION"));
